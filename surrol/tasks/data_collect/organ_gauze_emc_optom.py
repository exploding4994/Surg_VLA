...
class GauzeRetrieve(PsmEnv):
    ...
    def _env_setup(self):
        ...
        # Optional: Retain or adjust specular highlights
        p.changeVisualShape(obj_id2, -1, specularColor=(0.3, 0.3, 0.3))  # Softer highlights

        # === NEW: Disable collisions between PSM and the Spleen ===
        psm_body_id = self.psm1.body  # PSM body ID
        spleen_id = obj_id2  # Spleen body ID

        # Disable collisions between all PSM links (including base -1) and the spleen (base link -1)
        num_psm_joints = p.getNumJoints(psm_body_id)
        for link_idx in range(-1, num_psm_joints):  # -1 is the base link
            p.setCollisionFilterPair(
                bodyUniqueIdA=psm_body_id,
                bodyUniqueIdB=spleen_id,
                linkIndexA=link_idx,
                linkIndexB=-1,  # The spleen typically only has a single rigid body (base)
                enableCollision=0  # 0 = Disable collision
            )
        # ===================================
